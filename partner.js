var { numbers } = Qs.parse(location.search,{
    ignoreQueryPrefix: true
})
if(numbers == undefined) numbers = 5;
...
const form = document.getElementById("real-word-form")
const wholeForm = document.getElementById("word-form")
const wordInput = document.getElementById("word-input");
const wordbad = document.getElementById("word-message")
wordInput.setAttribute("maxlength", numbers )
form.addEventListener("submit", (e) => {
  e.preventDefault();
  const value = wordInput.value;
  if(dict.includes(value.toUpperCase())){
      realWord = value.toUpperCase();
      wholeForm.style.display = "none";
      doStuff();
  }else{
    wordbad.style.display = "block";
    setTimeout(() => {wordbad.style.display = "none"}, 1000)
  }
})
...
function doStuff(){
  ...
  // function setSize(){
  //     var poopy = (window.innerHeight * 0.5);
